...
def check_uidai():
    ...
    if score > 3:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM aadharkendra WHERE UIDAI=%s", (myuidai,))
        row = cursor.fetchone()
        if row is None:
            result_label.config(text="Aadhar card not found")
            add_button.config(state=tk.NORMAL)
        else:
            result_label.config(text="Aadhar card found")
...
def add_uidai():
    ...
    cursor = conn.cursor()
    cursor.execute("INSERT INTO aadharkendra (UIDAI) VALUES (%s)", (uidai,))
    conn.commit()
    conn.close()
    result_label.config(text="Aadhar card added to database")
...
def AddAadharcard():
    def submit_data():
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO aadharkendra (UIDAI, Name, father_name, MobileNo, DOB, Aadhar_card) VALUES (%s, %s, %s, %s, %s, %s)",
    ...
    aadhar_file_label = tk.Label(root, text="Aadhar Card:")
    aadhar_file_label.pack()
    file_path = tk.StringVar()
    file_selection_button = tk.Button(root, text="Select Aadhar Card", command=lambda: file_path.set(filedialog.askopenfilename()))
...
def update_data(myuidai):
    ...
    # Update the database
        ...
        sql = "UPDATE aadharkendra SET Name = %s, DOB = %s, father_name = %s, MobileNo = %s, Aadhar_card = %s WHERE UIDAI = %s"
...
# Create Add button
# Create Add button
add_button = tk.Button(root, text="Add Aadhar Card",
